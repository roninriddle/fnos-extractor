...
**本次更新**:
- 🧭 统一扫描路径与指定解压路径逻辑，扫描路径变更时自动建议 `<扫描目录>/extracted`
- 💾 记住最近使用的扫描目录、指定解压目录和解压模式，页面重新打开后自动恢复
- 📂 设置弹窗、目录树、子目录跳转都接入统一路径状态，避免多个入口互相脱节
- 🛡️ 用户手动修改过指定解压目录后不再自动覆盖，保留明确的自定义输出路径
- ✅ 本版针对 GitHub issue #2 整理为正式版 1.3.27
...
**Last Updated**: 2026-06-14 | Version: 1.3.27
